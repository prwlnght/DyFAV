...
constant_database_location <<- '/Users/lizard/Google Drive/School/Research/Projects/Hand/Data/IUIDemo_Data/'
...
recognized_gestures <<- c("vertical", "horizontal")

for ( matrix_creation_index in 1:17){
...
for (file in filenames_all){
  ...
  #zero the first init value
  temp <- data.matrix(temp)
  temp <- sweep(temp, 2, temp[1,])
  ...
  temp <- temp[1:MIN,]
  for (train_matrix_population_index in 1:17){
...
recognition_module <- function (testDataFile, this_sign){
  setwd('/Users/lizard/Google Drive/School/Research/Projects/Hand/Data/IUIDemo_Data/test/')
  filename <<- list.files(pattern ="csv", full.names=TRUE)
  
  testData <- read.csv(filename[1])
  
  #zero the first init value
  testData <- testData[1:17]
  testData <- data.matrix(testData)
  testData <- sweep(testData, 2, testData[1,])
  
  #normalize 
  #testData  <- apply(testData, MARGIN = 2, FUN = function(X) normalize(X))
  ...
  par(bg='blue4')
  plot(0)
  text(1,0, recognized_word, col="white", cex=11)
...
for(trial_index in 1:10){  
  ...
  system('/Users/lizard/Development/gitRepos/Hands/Data/EMGData/gatherDataRight.sh DemoData')
  
  setwd('/Users/lizard/Google\ Drive/School/Research/Projects/Hand/Data')
  system('rm IUIDemo_Data/test/*.csv')
  system('mv Alphabet/Pending/*.csv IUIDemo_Data/test/')
  ...
  ##athis_test_file <- paste(getwd(), this_test_file, sep='/')
  
  
  if(!exists('is_init')) this_initialize()
  start_time <<- Sys.time()
  
  recognition_module(this_sign, list_for_demo[trial_index] )
  #end_time <<- Sys.time()
